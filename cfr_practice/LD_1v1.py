...
# -----------------------------------------------
'''
for initialRoll in range(1, LD.sides + 1):
    print("Initial claim policy with roll {0:d}: ".format(initialRoll), end = '')
    for prob in LD.claimNodes[0][initialRoll].getAverageStrategy():
        print('{0:.2f} '.format(prob), end = '')
    print('')

print('\nOld_Claim\tNew_Claim\tAction_Probabilities')
for myClaim in range(0, LD.sides + 1):
    for oppClaim in range(myClaim + 1, LD.sides + 1):
        print('{0:d}\t{1:d}\t'.format(myClaim, oppClaim) + \
                str([float('%.4g' % float('%.3f' % x)) for x in \
                    LD.responseNodes[myClaim][oppClaim].getAverageStrategy()]))

print('\nOld_Claim\tRoll\tAction_Probabilities')
for oppClaim in range(0, LD.sides):
    for roll in range(1, LD.sides + 1):
        print('{0:d}\t{1:d}\t'.format(oppClaim, roll) + \
                str([float('%.3g' % float('%.2f' % x)) for x in \
                    LD.claimNodes[oppClaim][roll].getAverageStrategy()]))
'''
